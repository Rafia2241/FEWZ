...
python scripts/create_parallel_atlxplus.py $BOSON $INFILE $RUNDIR
...
if ! [ -e $RUNDIR/$HISTFILE ] ; then
    python scripts/get_bin_files_atlxplus.py $HISTFILE $RUNDIR
    if [ -d "$RUNDIR/${RUNDIR}0/pscale" ] || [ -d "$RUNDIR/${RUNDIR}0/mscale" ] ; then
        cat $RUNDIR/$HISTFILE | sed -e "s/'\.\.\//'..\/..\//g" > $RUNDIR/pm_$HISTFILE
    fi
fi
python scripts/create_condor_atlxplus.py $BOSON ${RUNDIR##*/} $INFILE $HISTFILE $OUTFILE $PDFDIR
cd $RUNDIR
...
if [ $# -le 6 ] ; then
   ...
   cd ..
   python scripts/create_condor_atlxplus.py $BOSON ${RUNDIR##*/} $INFILE $HISTFILE $OUTFILE $PDFDIR $JOBFILE $INIT_SECT $LAST_SECT $SECT_STEP
   cd $RUNDIR
