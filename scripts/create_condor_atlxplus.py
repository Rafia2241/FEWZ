...
try:
    ...
    if len(sys.argv) > 8:
        for i in sectlist:
	    job_file.write('Executable =' +outputdir+ '%i' +'/wrapper.sh\n' % (i-1))
            job_file.write('Arguments = %i\n' % (i-1))
            job_file.write('initialdir = ' + outputdir + '%i\n' % (i-1))
            job_file.write('when_to_transfer_output = ON_EXIT \n')
            job_file.write('+JobFlavour = "nextweek" \n')
            job_file.write('queue\n')
	    if (boson == 'W'):
		job_file.write('transfer_input_files = ../fewzw, ../' + inputfile + ', ../' + histofile \
				+ ', ../bins.txt, \n' )
	    else:
		if (boson != 'Z'):
	            print('Warning: unrecognized parameter; defaulting to neutral current\n')
		job_file.write('transfer_input_files = ../fewzz, ../' + inputfile + ', ../' + histofile \
				+ ', ../bins.txt, \n' )
    else:
	if (boson == 'W'):
	    job_file.write('transfer_input_files = ../fewzw, ../' + inputfile + ', ../' + histofile \
	    ...
	    job_file.write('transfer_input_files = ../fewzz, ../' + inputfile + ', ../' + histofile \
		+ ', ../bins.txt,\n' )
	job_file.write('Executable =' +outputdir +'$(Process)/wrapper.sh\n')
        job_file.write('Arguments = $(Process) \n')
